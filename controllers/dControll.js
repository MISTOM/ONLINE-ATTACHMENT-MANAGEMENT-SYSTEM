...
const { genPDF } = require('./helpers/pdfConfig');
...
function send2FACode (user) {
  ...
  const emailbody = `<h1>Dear ${user.first_name}</h1><br>
  ...
  const emailText = `<h1>Dear ${user.first_name}</h1><br>
  <h3>The 2 Factor Authentication for your OAMS Account has been successfully set.</h3><hr>
  <h3>Your one time code is <pre>${code}</pre> Valid for one minute</h3>
  <p>If you did not enable 2FA, please ignore this email and <a href="#">Secure your account</a></p>
  <small>Thank You; Regards OAMS Management.</small>`;
...
exports.dView = async (req, res, next) => {
  ...
  if (req.user.role_id === 1) {
    ...
    const link = await genPDF(req, res, next);
...
exports.profilePageView = (req, res, next) => {
  ...
  conn.query(`SELECT * FROM users us INNER JOIN user_profiles up ON us.user_id = ${id} AND up.user_id = ${id} LEFT JOIN institution_info insf ON insf.student_id =${id} LEFT JOIN institution_supervisor insv ON insv.institution_id=insf.institution_id INNER JOIN academic_year ay ON ay.academic_year_id = us.academic_year_id INNER JOIN programme prg ON up.programme_id = prg.programme_id`,
    (err, rows) => {
      ...
      const date1 = rows[0].from_date;
      const date2 = rows[0].to_date;
        ...
        (err, rows1) => {
          ...
          conn.query(`SELECT * FROM activities_table WHERE user_id = ${req.user.user_id} ORDER BY log_id DESC`, (err, rows2) => {
            ...
            for (const i in rows2) {
              const fdate = formatDate(rows2[i].log_date);
              rows2[i].fdate = fdate;
            }
            // console.log(rows2)
...
exports.logbook = (req, res, next) => {
  ...
  const q = 'INSERT INTO activities_table (user_id, student_logs, log_date) VALUES (?, ?, ?)';
  const v = [`${id}`, `${dayslog}`, `${today}`];
...
exports.attachForm = (req, res, next) => {
  ...
  //= =============================QUERY 1=====================================================================
  const q1 = 'INSERT INTO institution_info (student_id, institution_name, location, from_date, to_date, work_position, website, email_address, additional_info, letter_file) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';

  const v1 = [`${req.user.user_id}`, `${req.body.institution}`, `${req.body.location}`, `${req.body.startDate}`, `${req.body.endDate}`, `${req.body.position}`, `${req.body.website}`, `${req.body.email}`, `${req.body.moreInfo}`, `${req.file.filename}`];

  //= =============================QUERY 2=====================================================================
  const q2 = 'INSERT INTO institution_supervisor (supv_first_name, supv_last_name, supv_email, supv_contact, institution_id) VALUES (?, ?, ?, ?, (SELECT MAX(institution_id) from institution_info))';

  const v2 = [`${req.body.supFirstName}`, `${req.body.supLastName}`, `${req.body.supEmail}`, `${req.body.supContact}`];
    ...
    (err, result) => {
      ...
      conn.query(`SELECT * FROM institution_supervisor isup INNER JOIN institution_info insf ON insf.institution_id = isup.institution_id AND insf.student_id = ${Aid}`,
        (err, rows) => {
          ...
          const sid = `sup-no-${rows[0].institution_supervisor_id}`;
          const sfst = rows[0].supv_first_name;
          const slst = rows[0].supv_last_name;
          const seml = rows[0].supv_email;
          const scont = rows[0].supv_contact;
          const role = 2;

          const institution = rows[0].institution_name;
          // ________________________________________________________________________/

          const q = 'INSERT INTO users (first_name, last_name, role_id, registration_number, username) VALUES (?, ?, ?, ?, ?)';
          const v = [`${sfst}`, `${slst}`, `${role}`, `${sid}`, `${sfst}`];
          ...
          conn.query(q, v, (err, results) => {
            ...
            const q2 = 'INSERT INTO user_profiles (user_email, phone_number, user_id) VALUES (?, ?, (SELECT MAX(user_id) FROM users))';
            const v2 = [`${seml}`, `${scont}`];
            ...
            conn.query(q2, v2, (err, results) => {
              ...
              conn.query(`SELECT * FROM users WHERE user_id = ${Aid}`,
                (err, rows) => {
                  ...
                  const output = `<h1>Dear ${sfst} - ${institution}</h1><br>
                              ...
                              <small>Thank You; Regards OAMS Management.</small>`;
                  const mailtext = `<h1>Dear ${sfst} - ${institution}</h1><br>
                  <h3>The username for your OAMS Account is ${seml}.</h3><hr>
                  <h3>The password is ${sid}.</h3>
                  <p>Follow this link to login as ${rows[0].first_name} ${rows[0].last_name} ${rows[0].other_name}'s Supervisor =>: <a href="http://96.126.122.21">Online Attachment Management System</a></p>
                  <p>If you are not the supervisor at ${institution}, please ignore the email</p>
                  <small>Thank You; Regards OAMS Management.</small>`;
                  const mailSubject = 'Credentials of the Online Attachment System';
...
exports.supervisor = (req, res, next) => {
  if (req.user == undefined) return res.redirect('/');
  const supregNo = req.user.registration_number;
  const supNo = supregNo.split('-').pop();

  const q = `SELECT * FROM activities_table WHERE user_id IN (SELECT student_id FROM institution_info insf, institution_supervisor insv WHERE insf.institution_id = insv.institution_id AND insv.institution_supervisor_id = ${supNo}) ORDER BY log_id DESC`;
  conn.query(q, (err, rows) => {
    if (err) throw err;
    // _____FORMATING THE LOG DATE___
    for (const i in rows) {
      const fdate = formatDate(rows[i].log_date);
      rows[i].fdate = fdate;
    }
...
exports.supComment = (req, res, next) => {
  ...
  console.log(req.body);
  const q = 'UPDATE `activities_table` SET `supervisors_logs`= ' + '"' + supervisorLog + '"' + ' WHERE `log_date`=' + '"' + logdate + '"' + ' AND `user_id`=' + userid + '';
